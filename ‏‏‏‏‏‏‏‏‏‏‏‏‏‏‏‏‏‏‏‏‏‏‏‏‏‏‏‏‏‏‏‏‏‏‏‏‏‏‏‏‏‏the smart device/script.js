...
const appPages = [
    ...
    // Page 2
    [
        { name: 'Records', emoji: '🎙️', content: () => '<div class="app-content">Records App</div>' },
...
function initGame() {
    ...
    function endGame() {
        ...
        showWatchNotification(`Game Over! Score: ${finalScore}`);
...
function getGamesContent() {
    // Get the high score from localStorage or default to 0
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('.lamp-emoji').addEventListener('click', function() {
        ...
        if (!isLightOn) {
            // Create light options if they don't exist
            if (!lightOptions) {
            ...
            screen.style.setProperty('--brightness-level', '1.2');
            setTimeout(() => {
                lightOptions.classList.add('active');
            }, 500);
        } else {
            lightEffect.classList.remove('active');
            screen.style.setProperty('--brightness-level', '1');
            if (lightOptions) {
...
// Add this function to handle the settings window
function createSettingsWindow() {
    ...
    window.innerHTML = `
        ...
        <div class="window-content">
            ...
            <div class="settings-section">
                <h3>Sound</h3>
                <div class="settings-item">
                    <label for="windowVolume">Volume</label>
                    <input type="range" id="windowVolume" min="0" max="100" value="50">
                </div>
            </div>
        </div>
    `;

    document.querySelector('.computer-screen').appendChild(window);
    ...
    const brightnessSlider = window.querySelector('#windowBrightness');
    brightnessSlider.addEventListener('input', (e) => {
        const brightness = e.target.value;
...
// Update the taskbar click handler
document.addEventListener('DOMContentLoaded', () => {
    ...
    taskbarApps.forEach(app => {
        app.addEventListener('click', function() {
            ...
            // Add active state to clicked app
...
// Add this function to create a notes window
function createNotesWindow() {
    // Remove any existing notes window
    const existingWindow = document.querySelector('.notes-window');
    if (existingWindow) {
        existingWindow.remove();
    }
    ...
    // Make window draggable
    const header = notesWindow.querySelector('.window-header');
    let isDragging = false;
    let currentX;
    let currentY;
    let initialX;
    ...
    function drag(e) {
        if (isDragging) {
            ...
            notesWindow.style.transform = `translate(calc(-50% + ${currentX}px), calc(-50% + ${currentY}px))`;
        }
    }
    ...
    notesWindow.querySelector('.close').addEventListener('click', () => {
        notesWindow.classList.remove('active');
        setTimeout(() => notesWindow.remove(), 300);
    });

    notesWindow.querySelector('.minimize').addEventListener('click', () => {
        notesWindow.classList.remove('active');
    });

    notesWindow.querySelector('.maximize').addEventListener('click', () => {
        notesWindow.classList.toggle('maximized');
    });

    // Add note functionality
    notesWindow.querySelector('.add-note').addEventListener('click', createNote);
    
    notesWindow.querySelectorAll('.delete-note').forEach(button => {
        button.addEventListener('click', (e) => {
            const noteId = parseInt(e.target.closest('.note').dataset.id);
            deleteNote(noteId);
            createNotesWindow(); // Refresh the window
        });
    });

    notesWindow.querySelectorAll('.edit-note').forEach(button => {
        button.addEventListener('click', (e) => {
            const noteId = parseInt(e.target.closest('.note').dataset.id);
            editNote(noteId);
        });
